...
public class AudioAnalyzer {
    ...
    public static SampledAudio fft(SampledAudio x) throws AudioAnalyzerException {
        int N = x.size();
        ...
        if (N % 2 != 0) { 
            throw new AudioAnalyzerException("SampledAudio`s size must be a power of 2"); 
        }
    ...
    public static SampledAudio ifft(SampledAudio x) throws AudioAnalyzerException {
        int N = x.size();
    ...
    public static SampledAudio hammingWindow(SampledAudio samples) {
        ...
        for (int i = 0; i < samples.size(); ++i) {
            tmp.add(new Complex(samples.get(i).abs() * (0.54 - 0.46 * Math.cos(2 * Math.PI * i / (samples.size() - 1))), 0));
        }
        return tmp;
    }
    
    public static SampledAudio cepstrum(SampledAudio samples) throws AudioAnalyzerException {
    ...
    public static double averagePitch(WavFile file, int windowSize) throws AudioAnalyzerException {
        ...
        while (N != 1) {
            if (N % 2 == 1)
                throw new AudioAnalyzerException("windowSize must be a power of 2");
            N >>= 1;
        }
        
        
        for (int i = 0; i + windowSize < samples.size(); i += windowSize >> 1, ++N) {
            S += AudioAnalyzer.pitch(samples.getSamplesInRange(i, i + windowSize));
        }
