...
public class VowelMatcher {
    ...
    public String match(WavFile file, String username) throws AudioAnalyzerException {
        class Counter {
            int count = 0;
            double[] pitch = new double[5];
            synchronized void increment() { count++; }
            synchronized int getCount() { return count; }
            synchronized void set(int index, double pitch_value) { pitch[index] = pitch_value; }
            synchronized double[] getPitch() { return pitch; }
        }
        ...
        //System.out.println("Fara rec : " + (java.util.Calendar.getInstance().getTimeInMillis() - start));
        ...
        for (int i = 0; i < vowels.length; ++i) {
            for (int j = 0; j < 5; ++j) {
                ...
                double tmp = database.getUserData(username).getFrameData(windowSize).getVowelData(vowels[i]).getAverage();
                tmp = (tmp - a) / (b - a) + 1;
                tmp -= pitch[j];
                tmp *=tmp;
                diff[i] += tmp;
        ...
        //System.out.println("Cu rec : " + (java.util.Calendar.getInstance().getTimeInMillis() - start));
        return vowels[index];    
    }
}
